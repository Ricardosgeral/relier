...
The ***relier*** *acquisition system* is prepared to collect data simultaneously from the following ***'low cost' sensors***
...
An exception is the *BME280* chip, which can be soldered directly to the custom PCB or using a cable with 4 pins (Dupont). 
This last option is preferable, because, ideally, to monitor the ambient air temperature/humidity/pressure, 
the *BME280* should be placed outside the AS box (and inside the box, but near large openings), 
to avoid the heat generated by the monitor and by raspberry Pi!


### Custom *PCB* design (shield for Raspberry Pi)
